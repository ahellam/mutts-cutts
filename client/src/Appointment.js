...
function Appointment({appointment, handleDelete}) {
  ...
  return (
    <div className='border-2 grid grid-cols-10 shadow-md'>
      <img src={appointment.dog_image} className='h-20 rounded-xl p-1 w-[150px] object-cover'></img>
      ...
      <div className='p-1 self-center col-start-9 pl-4'><EditButton>Edit</EditButton></div>
    ...
    rounded
    hover:bg-red-600
    " onClick={() => handleDelete(appointment)}>Delete</button></div>
    </div>
  )
}
